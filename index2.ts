...
const researcherAgentCheckpointer = new MemorySaver();
const researcherAgent = createReactAgent({
  llm: agentModel,
  tools: researcherAgentTools,
  checkpointSaver: researcherAgentCheckpointer,
  messageModifier: new SystemMessage("You are a web researcher. You may use the AskNews search engine to search the web for" +
...
// Initialize memory to persist state between graph runs
const dbAgentCheckpointer = new MemorySaver();
const dbAgent = createReactAgent({
  llm: agentModel,
  tools: toolkit,
  checkpointSaver: dbAgentCheckpointer,
  messageModifier: new SystemMessage("You are a database administrator for the Chinook database.")
});

// Now it's time to use!
const agentFinalState = await researcherAgent.invoke(
...
const agentNextState = await dbAgent.invoke(
